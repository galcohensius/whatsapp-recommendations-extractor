# WhatsApp Recommendations Extractor

A tool to extract recommendations from WhatsApp chat messages and VCF contact files, then display them in an interactive, searchable web interface.

## Features

- 📱 **Extract from WhatsApp chats**: Parses WhatsApp chat exports (.txt files)
- 📇 **Process VCF contacts**: Extracts contact information from .vcf files
- 🔍 **Interactive web interface**: Searchable, filterable table with Hebrew (RTL) support
- 📊 **Multiple outputs**: Generates JSON and Markdown formats
- 📞 **Click-to-call**: Phone numbers are clickable links

## Project Structure

```
whatsapp-recommendations-extractor/
├── src/                          # Python scripts
│   ├── extract_recommendations.py
│   ├── generate_markdown_table.py
│   ├── cleanup_recommendations.py
│   └── utils.py
├── web/                          # Web interface
│   ├── index.html
│   └── recommendations.json      # Generated by extract script
├── output/                       # Generated outputs
│   └── recommendations.md        # Generated by markdown script
├── data/                         # Input data (gitignored)
│   ├── vcf/                     # VCF contact files
│   └── text/                    # WhatsApp chat exports
├── requirements.txt
└── readme.md
```

## Usage

### 1. Extract Recommendations

Run the extraction script to process WhatsApp chats and VCF files:

```bash
python src/extract_recommendations.py
```

This will:
- Parse all `.vcf` files from `data/vcf/`
- Parse WhatsApp chat from `data/text/`
- Generate `web/recommendations.json`

### 2. Generate Markdown Table

Generate a markdown table from the JSON:

```bash
python src/generate_markdown_table.py
```

This creates `output/recommendations.md` from `web/recommendations.json`.

### 3. Clean Up Recommendations

If you need to clean up invalid entries (e.g., URL parameters captured as names):

```bash
python src/cleanup_recommendations.py
```

## Running the Web Interface

To view the interactive web interface, you need to serve it through a local web server (required because the page loads data from `recommendations.json`).

### Option 1: Serve from web directory (Recommended)

```bash
cd web
python -m http.server 8000
```

Then open your browser and navigate to: `http://localhost:8000`

### Option 2: Serve from project root

```bash
# From project root
python -m http.server 8000
```

Then navigate to: `http://localhost:8000/web/`

### Option 3: Direct file opening (may not work due to CORS)

You can try double-clicking `web/index.html`, but this may not work in modern browsers due to CORS restrictions when loading JSON files.

## Setup

1. Install dependencies:
   ```bash
   pip install -r requirements.txt
   ```

2. Place your data files:
   - WhatsApp chat exports in `data/text/`
   - VCF contact files in `data/vcf/`

3. Run the extraction script to generate the recommendations.